...
class CoinScreenContentTest {
    ...
    private fun emptyTitleText() = composeRule.activity.getString(R.string.empty_coins_title)

    private fun emptyBodyText() = composeRule.activity.getString(R.string.empty_coins_body)

    private fun errorBodyText() = composeRule.activity.getString(R.string.error_message_body)

    private fun listContentDescription() = composeRule.activity.getString(R.string.coins_list_content_description)
    ...
    fun successState_displaysCoinsAndTopBar() {
        ...
        composeRule.onNodeWithContentDescription(listContentDescription()).assertIsDisplayed()
        composeRule.onNodeWithText("Litecoin").assertDoesNotExist()
        composeRule.onNodeWithText(retryText()).assertDoesNotExist()
    }

    @Test
    fun successState_withEmptyCoins_displaysEmptyState() {
        composeRule.setContent {
            CoinScreenContent(
                state =
                    State(
        ...
        composeRule.onNodeWithText(emptyTitleText()).assertIsDisplayed()
        composeRule.onNodeWithText(emptyBodyText()).assertIsDisplayed()
        composeRule.onNodeWithText(retryText()).assertDoesNotExist()
    }
    ...
    fun errorState_showsRetryAndEmitsEvent() {
        ...
        composeRule.onNodeWithText(errorBodyText()).assertIsDisplayed()
        composeRule.onNodeWithText(titleText()).assertDoesNotExist()
